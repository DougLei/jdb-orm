...
public class DatabaseMetadataEntity {
	private static final Logger logger = LoggerFactory.getLogger(DatabaseMetadataEntity.class);
	private String name; // 数据库名
	private int databaseMajorVersion; // 数据库主版本
	
	public DatabaseMetadataEntity(Connection connection) throws SQLException {
		try {
	...
	private String extractDatabaseName(String JDBCUrl) {
		...
		if(b-1 <= a) 
			throw new OrmException("JDBCUrl=" + JDBCUrl + ", 无法从中截取到对应的数据库名称, 第一个冒号的下标="+a+", 第二个冒号的下标="+b);
